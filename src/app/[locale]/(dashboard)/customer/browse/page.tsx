  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          title={t("empty.title")}
          description={search ? t("empty.noResults", { search }) : t("empty.noRegistered")}
          icon={<span className="text-5xl">🔍</span>}
        />
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {professionals.map((pro) => (
            <ProfessionalCard key={pro.id} professional={pro} viewLabel={t("viewAvailability")} />
          ))}
        </div>
      )}
    </div>
...
function ProfessionalCard({ professional, viewLabel }: { professional: UserPublic; viewLabel: string }) {
  return (
